...
<form class="form-horizontal"   method="post" action="{{route('areas.update',['area'=>$area->id])}}" enctype="multipart/form-data">
...
<div class="form-group">
		    ...
		    <div class="col-lg-9 col-md-12 col-sm-12 {{ $errors->has('name')?'has-error':'' }}">
		        <input   name="name" value="{{ old('name',$area->name) }}"  class="form-control" id="name" placeholder="Name" type="text">
		        		        @if($errors->has('name'))
...
(function($) {
    ...
    $(document).on("change","#state_id",function() {
		...
		if (state_id)
		{
			getcities(state_id,{{ old("city_id",$area->city_id) }})
		}
		return false;
	});
	getcities("{{ old("state_id",$area->state_id) }}","{{ old("city_id",$area->city_id) }}")
