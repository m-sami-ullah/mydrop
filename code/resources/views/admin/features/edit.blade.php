...
<form class="form-horizontal"   method="post" action="{{route('packages.features.update',['feature'=>$feature->id,'package'=>$package->id])}}" enctype="multipart/form-data">
...
<div class="col-md-12">
        <div class="panel">
		    ...
		    <div class="col-lg-9 col-md-12 col-sm-12 {{ $errors->has('name')?'has-error':'' }}">
		        <input   name="name" value="{{ old('name',$feature->name) }}"  class="form-control" id="name" placeholder="Feature" type="text">
		        		        @if($errors->has('name'))
        ...
        <div class="form-group">
                ...
                <div class="col-lg-8">
                    <label class="radio-inline i-checks">
                        <input name="available" id="inlineRadio1" value="1" {{ $feature->available==1 ? 'checked':'' }} type="radio">
                        <i></i> Yes
                    </label>
                    <label class="radio-inline i-checks">
                        <input name="available" id="inlineRadio12" value="2" {{ $feature->available==2 ? 'checked':'' }} type="radio">
                        <i></i> No
                    </label>
